...
const Backgammon = Game({
  ...
  moves: {
    ...
    moveStone(G, ctx, at, dice) {
      ...
      if (!boarding.mayMoveTo(board, currentPlayer, at, dice)) {
        return { ...G };
      }
      ...
      // throw out
      if (!boarding.isMyColor(board, currentPlayer, to) && boarding.exactlyOne(board, to)) {
    ...
    phases: [
      {
        ...
        turnOrder: {
          ...
          next: (G, ctx) => {
            if (
              (isFirstTurn(ctx) && hasTwoDice(G.openDice)) ||
              (isFirstTurn(ctx) && ctx.currentPlayer === 'any')
            ) {
